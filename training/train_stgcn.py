...
# Flatten features for CNN input
...
class STGCN(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.conv1(x)
        out = self.relu(out)
        out = out.transpose(1, 2)  # back to (batch, seq_len, hidden_size)
        out = self.fc(out)
        return out

model = STGCN(input_size=num_nodes * features, hidden_size=64, output_size=num_nodes * features)
...
torch.save(model.state_dict(), "models/stgcn_weights.pth")
print("✅ ST-GCN training complete.")